...
function export_codes_by_eventoc($data)
{
	...
	if ($order->total_price >= 200000) {
		...
		$content = "ONECARD gui tang quy khach ma giam gia ". $code_wefit[0]->code." - menh gia 300.000 vnd - HSD: ". date('d-m-Y', strtotime("+30 days")) ." Tai Ung Dung Phong Tap WeFit - Goi tap 01 thang - Hotline: 1900 1748";
		if ($code_wefit)
...
// and you might want to convert to integer
		...
		if ($voucher_type == 1) {
			$content = "Evoucher " . $sms_code . "- " . number_format($item->event->price) . " vnd - HSD: " . date('d/m/Y', strtotime($export_detail[$key]->expired)) . " Tai " . stripUnicode($item->event->title) . " - Hotline:19001748";
		} else {
			$content = "Voucher " . stripUnicode($item->event->title) . " - " . number_format($item->event->price) . " vnd - HSD: " . date('d/m/Y', strtotime($export_detail[$key]->expired)) . " se duoc chuyen den dia chi cua quy khach - Hotline:19001748";
		}
