...
class OnecardTableExport_voucher extends JTable
{
    ...
    public function check()
    {
       ...
       // $this->list_templates = json_encode($this->list_templates);
			...
			foreach ($vouchers as $voucher) {
                ...
                if (!$is_exported) {
                    ...
                    if (count($exported) < $voucher->number) {
                        $not_enough = 1;
                        $voucher_detail = OnecardHelper::get_voucher_detail($voucher->voucher);
                        $voucher_not_enough .=  $voucher_detail->title." ";
                    }
                }
			...
			if ($not_enough == 1) {
				throw new Exception(' Không đủ code cho Voucher:"<b> ' . $voucher_not_enough . '</b>". Vui lòng kiểm tra số lượng code và hạn sử dụng.');
			}
